...
int main (int argc, char *argv[])
{
  ...
  /***************************************/
  for (int q = 0; q < N; q += 16) { // Row of Block
	for (int r = 0; r < N; r += 16) { // Collumn of Block		
	// Now Operate On Matrixs as per matrix1
		for (i = 0; i < N; i++) {
			for (int j = q; (j < N) && (j < (q + 16)); j++) {
				for (int k = r; (k < N) && (k < (r + 16)); k++) {
					count += a[i][k] * b[k][j];
				}
				//Store Count and Reset
				c[i][j] = count;
				count = 0;
			}
		}
